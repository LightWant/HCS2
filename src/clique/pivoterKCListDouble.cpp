...
std::vector<double> pivoter::run() {
    printf("pivoterKCListOrdered.cpp::run\n");
    g.initHash();
#ifdef DDEBUG
g.print();
#endif
...
double p = answers[k];
#endif
        for(ui v : nodes[0]) sg.pIdx[v] = sg.deg[0][v] = g.pIdx[v];
        for(ui v : nodes[0]) {
...
for(ui i = 0; i < C.size(); i++) {
    ui v = C[i];
    ui deg = sg.deg[0][v] - sg.pIdx[v];
    if(deg > maxDeg) {
        maxDeg = deg;
        maxV = v;
    }
}
std::vector<ui> cands(maxDeg);
cands.clear();
for(ui j = sg.pIdx[maxV]; j < sg.deg[0][maxV]; j++) {
    ui v = sg.pEdge[j];
    cands.push_back(v);
}
for(auto v : C) {
    if(v != maxV && !g.connectHash(maxV, v)) C2.push_back(v);
}
assert(C2.size() + cands.size() + 1 == C.size());
ui cSize = 1;
auto findMax = [&](std::vector<ui> & cands, bool buildC2) {
    ui cSize = 0;
    while(cands.size() > 0) {
        maxDeg = 0, maxV = cands[0];
        for(ui i = 0; i < cands.size(); i++) {
            ui v = cands[i];
            ui deg = 0;
            for(ui j = 0; j < cands.size(); j++) {
                ui w = cands[j];
                if(g.connectHash(v, w)) deg++;
            }
            if(deg > maxDeg) {
                maxDeg = deg;
                maxV = v;
        ...
        // if(maxDeg == 0) break;
        std::vector<ui> newCands(maxDeg);
        newCands.clear();
        for(ui i = 0; i < cands.size(); i++) {
            if(cands[i] == maxV) continue;
            if(g.connectHash(maxV, cands[i])) {
                newCands.push_back(cands[i]);
            }
            else if(buildC2) C2.push_back(cands[i]);
        }
        cands = newCands;
    }
...
cSize += findMax(cands, true);
assert(cSize + C2.size() == C.size());
ui C2Size = C2.size();
ui c2Size = findMax(C2, false);
printf("%u %u %u %u %u\n", u, C.size(), cSize, C2Size, c2Size);
// printf("%u %u %u\n", u, C.size(), cSize);
...
printf("%u ", C[i]);
            ...
            for(ui i = sg.pIdx[u]; i < sg.deg[deep][u]; i++) {
                ui v = sg.pEdge[i];
                if(v < u || v == pivot || g.connectHash(pivot, v)) continue;
#ifdef DDEBUG
if(uu == uuu) {
