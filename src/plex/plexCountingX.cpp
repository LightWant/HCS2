...
std::vector<ull> plexCountingX::run() {
#ifdef DDDEBUG
g.print();
#endif
    printf("plexCountingX.cpp::run\n");
...
std::cout<<"    start "<<u<<' '<<answers[q]<<std::endl;
        ...
        ui edR = 0;
        P.clear(); H.clear();
        R.changeTo(u, edR++); H.push_back(u);
        nn.addNonNei(u, u);
...
for(ui i = 0; i < edC; i++) {
    ...
    for(ui j = sg.pIdx[v]; j < sg.pIdx2[v]; j++) {
        ...
        bkPivot(1, edC, 0, 0, edR);

        for(ui i = 0; i < edC; i++) nn.clear(C[i]);
        for(ui i = 0; i < edR; i++) nn.clear(P[i]);
    }
#ifdef BASELINE
...
void plexCountingX::bkPivot(ui deep, ui edC, ui stX, ui edX, ui edR) {
#ifdef DDDEBUG
