 ...
 * step 1 , update neighnbors, only update neighbors in C X P : almost no effect
...
printf("continue %u\n", v);
        ...
        for(ui j = sg.pIdx[v]; j < sg.pIdx2[v]; j++) {
            ui u = sg.pEdge[j];
            if(C.isIn(u, 0, edC)) {
                ui cNei = edP - neighborsInP[u];
// if(cNei >= k) {
//     printf("u %u, neighborsInP[u] %u, edP %u\n", u, neighborsInP[u], edP);
//     for(ui j = 0; j < edP; j++) if(sg.connect2(u, P[j])) printf("%u ", P[j]);printf("\n");
//     printf("cNei %u, k %u\n", cNei, k);
//     printf("deep %u\n");
//     fflush(stdout);
// }
// assert(cNei < k);
        ...
        ui s = 0;
        for(ui j = 0; j < edP; j++) {
            support[j] = k - (edP - neighborsInP[P[j]]);
            s += k - (edP - neighborsInP[P[j]]);
        }
        ui up = edP + k - (edP - neighborsInP[v]) + bucket[0].size();
        for(ui j = 1; j < k; j++) {
            for(auto u: bucket[j]) {
                ui minJ = 0, minS = k + 1;
                for(ui l = 0; l < edP; l++) {
...
// assert(minS < k + 1);
        ...
        for(ui j = sg.pIdx[v]; j < sg.pIdx2[v]; j++) {
            ui u = sg.pEdge[j];
            if(P.isIn(u, 0, edP) || C.isIn(u, 0, edC) || X.isIn(u, stX, edX)) 
                neighborsInP.increment(u);
        }
